                                                   ...
                                                   LA::MPI::Vector MMA_Xmin)
{
    ...
    else
    {
        ...
        for (unsigned int i : MMA_Dofs_IndexSet)
        {
            ...
            Moving_Low_Boundary[i] = std::min<double>(Moving_Low_Boundary[i], MMA_Design_Variables[i] - 1.e-5 * Xmax_Xmin_Epsi_Temp);
            Moving_Upp_Boundary[i] = std::max<double>(Moving_Upp_Boundary[i], MMA_Design_Variables[i] + 1.e-5 * Xmax_Xmin_Epsi_Temp);
            Moving_Upp_Boundary[i] = std::min<double>(Moving_Upp_Boundary[i], MMA_Design_Variables[i] + 100. * Xmax_Xmin_Epsi_Temp);

            Xmax_Temp = MMA_Xmax[i] + 1.e-6;
            Xmin_Temp = MMA_Xmin[i] - 1.e-6;
...
void Deal_II_MMA_Solver::Get_Dual_Hess(LA::MPI::Vector MMA_Design_Variables)
{
    ...
    for (unsigned int i = 0; i < Number_Of_Constraints; i++)
    {
        HessCorr = 1e-4 * Object_Function_Hess[i][i] / Number_Of_Constraints;
    }
    if (-1. * HessCorr < 1.e-7)
    {
        HessCorr = -1.e-7;
