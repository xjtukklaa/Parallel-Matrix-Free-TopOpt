#include "../include/Deal_II_MMA_Parallel.h"
...
void Deal_II_MMA_Solver::Deal_II_MMA_Reinit(LA::MPI::Vector MMA_Design_Variables)
{
    // 参数初始化
    MMA_Dofs_IndexSet = MMA_Design_Variables.locally_owned_elements();
    // 移动上下限制Low,Upp,Alpha,Beta
...
void Deal_II_MMA_Solver::Deal_II_MMA_Dual_Problem_Solve(LA::MPI::Vector &MMA_Design_Variables,
...
double Deal_II_MMA_Solver::Deal_II_MMA_KKT_L2_Norm(LA::MPI::Vector MMA_Design_Variables,
                                                   LA::MPI::Vector MMA_Object_Function_Diff_Values,
                                                   Vector<double>  MMA_Constraints_Function_Values,
                                                   std::vector<LA::MPI::Vector> MMA_Constraints_Function_Diff_Values,
                                                   LA::MPI::Vector MMA_Xmax,
...
////////// 内置函数-不能更改
/////////////////////////////////////////////////////////////////////////////////////
double Deal_II_MMA_Solver::Deal_II_MMA_Get_Change(LA::MPI::Vector MMA_Design_Variables)
{
    double change = 0.;
    for (unsigned int i : MMA_Dofs_IndexSet)
    {
...
// MMA子函数构建
void Deal_II_MMA_Solver::Generate_SubConvexProblem(LA::MPI::Vector MMA_Design_Variables,
                                                   LA::MPI::Vector MMA_Object_Function_Diff_Values,
                                                   Vector<double> MMA_Constraints_Function_Values,
                                                   std::vector<LA::MPI::Vector> MMA_Constraints_Function_Diff_Values,
                                                   LA::MPI::Vector MMA_Xmax,
                                                   LA::MPI::Vector MMA_Xmin)
{
    ...
    if (Loop_Iter < 3)
    {
        Moving_Low_Boundary = MMA_Design_Variables;
        Moving_Low_Boundary.add(-Asym_Init, MMA_Xmax, Asym_Init, MMA_Xmin);
        Moving_Upp_Boundary = MMA_Design_Variables;
        Moving_Upp_Boundary.add(Asym_Init, MMA_Xmax, -Asym_Init, MMA_Xmin);
    }
    else
    {
    ...
    for (unsigned int i : MMA_Dofs_IndexSet)
    {
        ...
        // Alpha
        Moving_Alpha_Boundary[i] = std::max<double>(MMA_Xmin[i], 0.9 * Moving_Low_Boundary[i] + 0.1 * MMA_Design_Variables[i]);
        // Beta
        Moving_Beta_Boundary[i] =  std::min<double>(MMA_Xmax[i], 0.9 * Moving_Upp_Boundary[i] + 0.1 * MMA_Design_Variables[i]);
        // 按照符号把目标函数和约束函数的导数分开
        // Xmax_Xmin_Epsi_Inv = 1. / std::max(Xmax_Xmin_Epsi, MMA_Xmax[i] - MMA_Xmin[i]);
        Xmax_Xmin_Epsi_Inv = 1. / (Moving_Upp_Boundary[i] - Moving_Low_Boundary[i]);
        // Object Function
...
void Deal_II_MMA_Solver::Generate_XYZ_From_Lambda(LA::MPI::Vector &MMA_Design_Variables)
{
...
// 这个函数用来计算原始对偶系统的梯度
void Deal_II_MMA_Solver::Get_Dual_Gradient(LA::MPI::Vector MMA_Design_Variables)
{
    for (unsigned int j = 0; j < Number_Of_Constraints; j++)
    {
...
void Deal_II_MMA_Solver::Get_Dual_Hess(LA::MPI::Vector MMA_Design_Variables)
{
    double Object_Function_Positive_Mutiply_Lambda = 0;
    double Object_Function_Negative_Mutiply_Lambda = 0;
...
void Deal_II_MMA_Solver::Solve_Linear_System()
{
    if (Number_Of_Constraints > 1)
    {
        ...
        for (unsigned int col_index = 0; col_index < Number_Of_Constraints; col_index++)
        {
            ...
            {
                if (std::abs(Object_Function_Hess(row_index,col_index)) > Max_Row_Number)
                {
                    Max_Row_Number = std::abs(Object_Function_Hess(row_index,col_index));
...
double Deal_II_MMA_Solver::Get_Dual_Residual(LA::MPI::Vector MMA_Design_Variables,
                                             double Epsi)
{
    double Residual = 0.;
    double Error = 0.;
...
void Deal_II_MMA_Solver::Interior_Point_Solve(LA::MPI::Vector &MMA_Design_Variables)
{
    for (unsigned int i = 0; i < Number_Of_Constraints; i++)
    {
