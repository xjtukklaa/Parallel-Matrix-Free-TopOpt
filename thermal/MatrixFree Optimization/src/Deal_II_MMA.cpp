                                          ...
                                          MMAVectorType MMA_Design_Variables)
{
    ...
    // Moving bounds Low, Upp, Alpha, Beta
    ...
    // Gradient information corresponding to the MMA subproblem construction
    ...
    // Design variables of the previous two iterations stored by MMA
...
/////////////////////////////////////////////////////////////////////////////////////
////////// Internal functions - cannot be modified
/////////////////////////////////////////////////////////////////////////////////////
...
void Deal_II_MMA_Solver::Generate_SubConvexProblem(MMAVectorType MMA_Design_Variables,
                                                   ...
                                                   MMAVectorType MMA_Xmin)
{
    // MMA determines the current iteration number
    // If the loop is less than three times, the influence of previous iterations is not considered
    // If more than three generations, the influence of previous iterations needs to be considered
    // Compute Low and Upp bounds
    if (Loop_Iter < 3)
    {
        for (unsigned int i : MMA_Dofs_IndexSet)
        {
    ...
    else
    {
        ...
        for (unsigned int i : MMA_Dofs_IndexSet)
        {
            // (x-x_old1)*(x_old1-x_old2) determines whether the iteration sequence is converging or diverging
            Loop_Factor = (MMA_Design_Variables[i] - Old_Design_Variables[0][i]) *
                          (Old_Design_Variables[0][i] - Old_Design_Variables[1][i]);
            // If the iteration sequence converges (the direction of iteration change is consistent), the above product is positive
            // If the sequence diverges, the above product is negative
            if (Loop_Factor < 0.)
            {
            ...
            Xmax_Xmin_Epsi_Temp = std::max<double>(Xmax_Xmin_Epsi, MMA_Xmax[i] - MMA_Xmin[i]);
    ...
    // This parameter is used to prevent the derivatives of the subproblem from deviating too much from the original gradient
    ...
    for (unsigned int i : MMA_Dofs_IndexSet)
    {
        ...
        // Separate the derivatives of the objective function and constraint functions according to their signs
        Xmax_Xmin_Epsi_Inv = 1. / std::max<double>(Xmax_Xmin_Epsi, MMA_Xmax[i] - MMA_Xmin[i]);
        // Object Function
    ...
    // Compute the values of the constraint functions
    for (unsigned int j = 0; j < Number_Of_Constraints; j++)
    {
        SubProblem_B[j] = 0.;
        for (unsigned int i : MMA_Dofs_IndexSet)
...
// Based on dual conditions
// Compute X, Y, Z variable values after obtaining dual variables Lambda
...
// This function computes the gradient of the primal-dual system
...
// Backtracking line search to obtain step size
...
void Deal_II_MMA_Solver::Solve_Linear_System()
{
    if (Number_Of_Constraints > 1)
    {
        ...
        for (unsigned int col_index = 0; col_index < Number_Of_Constraints; col_index++)
        {
            Max_Row_Number = 0;
            Max_Row_Index = 0;
            // Select column pivot, obtain the maximum value of the column pivot and its corresponding index
            ...
            // If the maximum pivot is less than 1e-10, report an error
            Assert(abs(Max_Row_Number) <= 1e-10, ExcZero());
            // Swap the current row with the row of the maximum pivot
